...
const code = {
  "version": 3,
  "type": "kit",
  "search": {"url": "http://rutor.info/search/0/0/100/0/%search%", "method": "GET"},
  "auth": {},
  "selectors": {
    "row": {"selector": "#index > table > tbody > tr"},
    "skipFromStart": 1,
    "title": {"selector": "td:eq(1)>a:eq(-1)", "pipeline": [{"name": "getText"}]},
    "url": {"selector": "td:eq(1)>a:eq(-1)", "pipeline": [{"name": "getProp", "args": ["href"]}]},
    "size": {"selector": "td:eq(-2)", "pipeline": [{"name": "getText"}, {"name": "parseSize"}]},
    "downloadUrl": {"selector": "td:eq(1)>a:eq(-2)", "pipeline": [{"name": "getProp", "args": ["href"]}]},
    "seeds": {"selector": "td:eq(-1)>span.green", "pipeline": [{"name": "getText"}, {"name": "toInt"}]},
    "peers": {"selector": "td:eq(-1)>span.red", "pipeline": [{"name": "getText"}, {"name": "toInt"}]},
    "date": {
      "selector": "td:eq(0)",
      "pipeline": [{"name": "getText"}, {"name": "legacyReplaceMonth"}, {"name": "legacyParseDate", "args": ["1"]}]
    },
