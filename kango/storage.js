...
return false;}};kango.IConsole=function(){};kango.IConsole.prototype={log:function(str){throw new kango.NotImplementedException();}};kango.IIO=function(){};kango.IIO.prototype={isLocalUrl:function(url){return(url.indexOf(kango.SCHEME)==0||(url.indexOf('http://')==-1&&url.indexOf('https://')==-1));},getExtensionFileContents:function(filename){throw new kango.NotImplementedException();}};kango.IStorage=function(){};kango.IStorage.prototype={setItem:function(name,value){throw new kango.NotImplementedException();},getItem:function(name){throw new kango.NotImplementedException();},removeItem:function(name){throw new kango.NotImplementedException();},getKeys:function(){throw new kango.NotImplementedException();},clear:function(){throw new kango.NotImplementedException();}};kango.LangBase=function(){};kango.LangBase.prototype={getGlobalContext:function(){return((function(){return this;}).call(null));},invoke:function(context,methodName,params){var arr=methodName.split('.');var parent=context;var funcName=arr.pop();for(var i=0;i<arr.length;i++){parent=parent[arr[i]];}
return parent[funcName].apply(parent,params);},evalInSandbox:function(win,api,text){throw new kango.NotImplementedException();},clone:function(obj){return JSON.parse(JSON.stringify(obj));},isString:function(obj){return(typeof obj=='string'||obj instanceof String);},isObject:function(obj){return(Object.prototype.toString.call(obj)=='[object Object]'||typeof obj=='object');},isArray:function(obj){return(obj instanceof Array||Object.prototype.toString.call(obj)=='[object Array]');},isFunction:function(obj){return(typeof obj=='function');}};(function(){kango.oop.mixin(kango,kango.EventTarget.prototype);kango.oop.mixin(kango,new kango.EventTarget());kango.oop.mixin(kango,{_configFileName:'extension_info.json',_extensionInfo:null,_messageRouter:null,_modules:[],_loadExtensionInfo:function(){this._extensionInfo=JSON.parse(kango.io.getExtensionFileContents(this._configFileName));},_initMessaging:function(){var self=this;this._messageRouter=new kango.MessageRouter();this._messageRouter.onmessage=function(event){self.fireEvent(self.event.Message,event);};this.dispatchMessage=function(name,data){this._messageRouter.dispatchMessage(name,data);};},_initModules:function(){for(var i=0;i<this._modules.length;i++){(new this._modules[i]()).init(kango);}},_init:function(){this._loadExtensionInfo();this._initMessaging();this._initModules();return this.fireEvent(this.event.Ready);},registerModule:function(classObj){this._modules.push(classObj);},getExtensionInfo:function(){return kango.lang.clone(this._extensionInfo);},getContext:function(){var background=kango.backgroundScript.getContext();return background?background:kango.lang.getGlobalContext();},isDebug:function(){var info=this.getExtensionInfo();return(typeof info.debug!='undefined'&&info.debug);},SCHEME:'kango-extension://'});})();
...
http://kangoextensions.com/
*/
kango.TabProxy=function(tab){this.xhr=kango.xhr;this.console=kango.console;this.browser={getName:function(){return kango.browser.getName()}};this._tab=tab;this._listeners=[];(new kango.InvokeAsyncModule()).init(this);(new kango.MessageTargetModule()).init(this);};kango.TabProxy.prototype={_tab:null,_listeners:null,xhr:null,console:null,event:{Message:'message'},dispatchMessage:function(name,data){var event={name:name,data:data,origin:'tab',source:this._tab,target:this._tab};kango.fireEvent(kango.event.Message,event);},addEventListener:function(type,listener){if(type=='message'){for(var i=0;i<this._listeners.length;i++){if(this._listeners[i]==listener){return;}}
...
catch(e){return false;}},getKeys:function(){var count={};return this._preferenceBranch.getChildList('',count);},clear:function(){return this._preferenceBranch.deleteBranch('');}};kango.Storage=function(){var url='http://'+"kango".replace('_','-')+'.kangoextensions.com';var ios=Components.classes['@mozilla.org/network/io-service;1'].getService(Components.interfaces.nsIIOService);var ssm=Components.classes['@mozilla.org/scriptsecuritymanager;1'].getService(Components.interfaces.nsIScriptSecurityManager);var dsm=Components.classes['@mozilla.org/dom/storagemanager;1'].getService(Components.interfaces.nsIDOMStorageManager);var uri=ios.newURI(url,'',null);var principal=ssm.getCodebasePrincipal(uri);this._storage=dsm.getLocalStorageForPrincipal(principal,'');};kango.Storage.prototype=kango.oop.extend(kango.IStorage,{_storage:null,getItem:function(name){var value=this._storage.getItem(name);if(typeof value!='undefined'){return JSON.parse(value);}
...
return keys;}});kango.storage=new kango.Storage();kango.addEventListener(kango.event.Uninstall,function(){window.addEventListener('beforeunload',function(){kango.storage.clear();},false);});(function(){var prefStorage=new kango.PrefStorage();var keys=prefStorage.getKeys();if(keys.length>0){for(var i=0;i<keys.length;i++){kango.storage.setItem(keys[i],prefStorage.getItem(keys[i]));}
prefStorage.clear();}})();
