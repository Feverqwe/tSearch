...
tracker[tmp_num] = function () {
    ...
    var web = function () {
        ...
        var calculateTime = function (time) {
            ...
            time = time[0].split(':');
            return Math.round((new Date(parseInt('20'+date[2]),parseInt(date[1])-1,parseInt(date[0]),parseInt('1'+time[0])-100,parseInt('1'+time[1])-100)).getTime() / 1000);
        }
        var readCode = function (c) {
            ...
            for (i = 1;i<l;i++) {
                ...
                if (td.eq(6).children('a').attr('href') == null) continue;
                arr[arr.length] = {
                    'category' : {
