...
tracker[tmp_num] = function () {
    ...
    var web = function () {
        ...
        var calculateTime = function (f) {
            var dd = f.split('.');
            return Math.round(new Date(parseInt(dd[2]),parseInt('1'+dd[1])-101,parseInt('1'+dd[0])-100).getTime() / 1000);
        }
            ...
            for (i = 1;i<l-1;i++) {
                ...
                arr[arr.length] = {
                    'category' : {
                        ...
                        'id': calculateCategory(td.eq(1).children('a').attr('href').replace(/(.*)f=([0-9]*)/i,"$2"))
                    },
                    'title' : td.eq(2).children('a').text(),
                    'url' : root_url+td.eq(2).children('a').attr('href'),
                    'size' : calculateSize(td.eq(5).text()),
